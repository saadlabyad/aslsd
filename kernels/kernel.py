...
class KernelModel():
    ...
    def get_l2_projection_loss(self, params, kernel_2, ker_param_2):
        return self.l2_norm(params)-2*self.l2_dot(kernel_2, params, ker_param_2)

    def diff_l2_projection_loss(self, params, kernel_2, ker_param_2):
        n_param = len(params)
        grad = np.zeros(n_param)
        for ix_param in range(n_param):
            grad[ix_param] = self.diff_l2_norm(ix_param, params)-2.*self.diff_l2_dot(kernel_2, 1, ix_param, params, ker_param_2)
        return grad

    def get_l2_projection(self, kernel_2, ker_param_2, n_iter=1000, params_0=None,
        ...
        for t in tqdm(range(n_iter), disable=not verbose):
            ...
            if log_error:
                l2_err_log[t] = self.get_l2_projection_loss(params, kernel_2, ker_param_2)
