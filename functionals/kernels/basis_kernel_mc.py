...
from aslsd.functionals.kernels.basis_kernels.preloaded_mc.\
    preloaded_mc import dict_preloaded
...
class BasisKernelMC():
    ...
    def make_l2_norm(self, vars_):
        omega = vars_[0]
        sim_tau = self.src_simu_func(self.rng, vars_[1:], size=self.n_mc_l2)
        return (omega**2)*(np.mean(self.tphi_func(sim_tau, vars_[1:])))

    def make_diff_l2_norm(self, ix_diff, vars_):
        omega = vars_[0]
        if ix_diff == 0:
            sim_tau = self.src_simu_func(self.rng, vars_[1:], size=self.n_mc_l2)
            return 2.*omega*(np.mean(self.tphi_func(sim_tau, vars_[1:])))
        else:
            pass

    def make_l2_dot(self, basis_kern_2, vars_1, vars_2):
        omega = vars_1[0]
        sim_tau = self.src_simu_func(self.rng, vars_1[1:], size=self.n_mc_l2)
        return omega*np.mean(basis_kern_2.make_phi(sim_tau, vars_2))
    ...
    def make_diff_l2_dot(self, basis_kern_2, ix_func, ix_diff, vars_1, vars_2):
        ...
        if ix_func == 1:
            if ix_diff == 0:
                return np.mean(basis_kern_2.make_phi(sim_tau, vars_2))
            else:
                pass
        elif ix_func == 2:
            return omega*np.mean(basis_kern_2.make_diff_phi(sim_tau, ix_diff,
                                                            vars_2))
